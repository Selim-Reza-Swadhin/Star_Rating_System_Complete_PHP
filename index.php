...
if (isset($_POST['rating_hidden_button'])) {
    ...
    $query = "insert into  rating_feedback(rating, feedback, email, image, rating_time)VALUES (?,?,?,?,?)";
    $stmt = $mysqli->prepare($query);
    $image = 'blog-man4.png';
    $date_time = date('Y-m-d H:i:s');
    $stmt->bind_param('issss', $rating, $feedback, $email, $image, $date_time);
    $stmt->execute();
...
<div class="container">
    <div class="row">
        <div class="com-md-6 col-md-offset-3">
            ...
            <hr style="background-color: #0f0f0f !important;">
            ...
            <br>
            Averaged Point : <?= getAverageRating(); ?>
            <hr class="bg-primary">
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <div class="media">
                            <div class="media-left">
                                <a href="">
                                    <img class="media-object" style="width: 50px;height: 55px;" src="images/<?= $row["image"]; ?>"
                                         alt="no image">
                                </a>
                            </div>
                            <div class="media-body">
                                ...
                                <?= $row['feedback']; ?> <br>
                                By : <a href=""><?= $row['email']; ?></a><br>
                                Time:<?= date('Y-m-d h:i:s a', strtotime($row['rating_time'])); ?>
                            </div>
                        </div>
                    <?php }
                }
